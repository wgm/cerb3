 ...
 *
 */
class ticket_assign_team_handler extends xml_parser
{
   /**
    ...
    *
    * @param object $xml
    * @return ticket_assign_team_handler
    */
   function ticket_assign_team_handler(&$xml) {
      $this->xml =& $xml;
   }
    ...
    */
	function process() {
      ...
      $email_obj =& new email_tickets();
		...
		if($email_obj->assign_team($ticket_id, $team_id) === FALSE) {
			xml_output::error(0, 'Failed assigning team to ticket.');
		}
		else {
			xml_output::success();
		}
