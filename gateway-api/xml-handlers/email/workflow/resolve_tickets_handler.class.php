 ...
 *
 */
class resolve_tickets_handler extends xml_parser
{
   /**
    ...
    *
    * @param object $xml
    * @return resolve_tickets_handler
    */
   function resolve_tickets_handler(&$xml) {
      $this->xml =& $xml;
   }
   ...
   function process() {
      ...
      if($email_obj->resolve_tickets($tickets, $agent_id) === FALSE) {
         xml_output::error(0, 'Resolve tickets failed'); 
      }
      else {
         xml_output::success();
      }
...
<cerberus_xml>
   ...
   <command>resolve_tickets</command>
   <data>
   <agent id="1"/>
   <tickets>
	  <ticket id="2"/>
