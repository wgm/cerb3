...
define("DB_SCRIPT_NAME","Cerberus Helpdesk 3.6 New Install");
...
function cer_init()
{
	...
	// [JAS]: Ensure everything runs
	foreach($procs as $proc) {
		if(false === call_user_func($proc)) {
//			echo "<br><font color='red'>A problem was encountered.</font><br>";
			return false;
		}
	}
...
function table(&$TABLE_DEF)
{
	...
	if(!$TBL->check(false)) {
		$result = create_table($TBL,$TABLE_DEF); // create
...
{
	$result = $TBL->run_sql($TABLE_DEF->create_sql,sprintf("<b>Creating table `%s`</b>",$TABLE_DEF->table_name));
	if(!$result) return false;
	$TBL->_read_table();
	return true;
}
...
function set_precursor_hashes()
{
	...
	if($TABLE_PM->table_exists) {
		...
		$cerberus_db->query($sql);
		$sql = "REPLACE INTO db_script_hash(script_md5,run_date) VALUES('0de5859da177a35773dffe066fbb38ce',NOW())"; // 3.4 to 3.5 Release
		$cerberus_db->query($sql);
		$sql = "REPLACE INTO db_script_hash(script_md5,run_date) VALUES('84167fd61544d6d3c3d3b97ced03b441',NOW())"; // 3.5 to 3.6 Release
		$cerberus_db->query($sql);
		$sql = "REPLACE INTO db_script_hash(script_md5,run_date) VALUES('a646f96e8f3bd6f7ced1737d389b1239',NOW())"; // 3.6 upgrade... this shouldn't have to be here.
		$cerberus_db->query($sql);
	}
...
function init_table_configuration() {
	...
	$TABLE_DEF->create_sql = "CREATE TABLE `configuration` (".
								...
								"  `smtp_server_user` char(64) NOT NULL default '',".
								"  `smtp_server_pass` char(64) NOT NULL default '',".
								"  `cut_line` char(64) NOT NULL default '',".
	...
	$TABLE_DEF->fields["smtp_server_user"] = new CER_DB_FIELD("smtp_server_user","char(64)","","","","");
	$TABLE_DEF->fields["smtp_server_pass"] = new CER_DB_FIELD("smtp_server_pass","char(64)","","","","");
	$TABLE_DEF->fields["cut_line"] = new CER_DB_FIELD("cut_line","char(64)","","","","");
...
function init_table_thread()
{
	...
	$TABLE_DEF->create_sql = "CREATE TABLE `thread` (".
								...
								"  `thread_to` char(255) default '',".
								"  `thread_cc` char(255) default '',".
								"  `thread_bcc` char(255) default '',".
	...
	$TABLE_DEF->fields["thread_to"] = new CER_DB_FIELD("thread_to","char(255)","YES","","","");
	$TABLE_DEF->fields["thread_cc"] = new CER_DB_FIELD("thread_cc","char(255)","YES","","","");
	$TABLE_DEF->fields["thread_bcc"] = new CER_DB_FIELD("thread_bcc","char(255)","YES","","","");
	...
	$TABLE_DEF->fields["is_hidden"] = new CER_DB_FIELD("is_hidden","tinyint(4)","","","0","");
...
function init_table_ticket()
{
	...
	$TABLE_DEF->create_sql = "CREATE TABLE `ticket` (".
								...
								"  `last_wrote_address_id` bigint(20) unsigned NOT NULL default '0',".
								"  `ticket_last_date` datetime NOT NULL default '0000-00-00 00:00:00',".
	...
	$TABLE_DEF->fields["ticket_status_id"] = new CER_DB_FIELD("ticket_status_id","tinyint(4) unsigned","","","0","");
	...
	$TABLE_DEF->fields["last_wrote_address_id"] = new CER_DB_FIELD("last_wrote_address_id","bigint(20) unsigned","","MUL","0","");
	$TABLE_DEF->fields["ticket_last_date"] = new CER_DB_FIELD("ticket_last_date","datetime","","MUL","0000-00-00 00:00:00","");
	...
	$TABLE_DEF->indexes["last_wrote_address_id"] = new CER_DB_INDEX("last_wrote_address_id","1","last_wrote_address_id");
	$TABLE_DEF->indexes["ticket_last_date"] = new CER_DB_INDEX("ticket_last_date","1","ticket_last_date");
...
function init_table_user_prefs()
{
	...
	$TABLE_DEF->create_sql = "CREATE TABLE `user_prefs` (".
								...
								"  `quote_previous` tinyint(1) NOT NULL default '1',".
	...
	$TABLE_DEF->fields["quote_previous"] = new CER_DB_FIELD("quote_previous","tinyint(1)","","","1","");
